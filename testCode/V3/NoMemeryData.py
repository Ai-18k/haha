...
class NoMemerySpider(Config):
        ...
        while True:
            try:
                ...
                if data["state"] == "ok":
                    ...
                    while True:
                        res = self.conn.lpop("NoMemeryCookie")
                        if res is None:
                            print(">>>>>>>>>>>>>>获取cookie信息..........")
                            time.sleep(0.5)
                        else:
                ...
                if data["state"] == "ok":
                    ...
                    if response.status_code == 200:
                        for item in data['data']['list']:
                            ...
                            logger.success(f"【*】行政许可 数据保存成功:{data}!!")
                        page += 1
                else:
                    while True:
                        res = self.conn.lpop("NoMemeryCookie")
    ...
    def send_data(self, flg, item):
        # 使用公司名称和证书编号作为唯一标识
        company_name = item.get('relationCompanyName')
        cert_number = item.get('certificateNumber') or item.get('licenceNumber') or item.get('propertyNum') or ''
        unique_key = f"{company_name}_{cert_number}"
        
        if unique_key in self.f_map[str(flg)]:
            logger.info(f"跳过重复记录: {unique_key}")
            return  # Skip processing if already processed
        
        print(f">>>>>>> %s 记录打点: %d " % (flg, len(self.f_map[str(flg)])))
        self.f_map[str(flg)].add(unique_key)

        # 将数据添加到线程本地存储的列表中
        if not hasattr(self.thread_local, 'data_buffer'):
            self.thread_local.data_buffer = {
                '4': [],  # sbxx_list
                '5': [],  # zpzzq_list
                '6': []   # rzzzq_list
            }
        ...
        # 添加数据到对应的缓冲区
        self.thread_local.data_buffer[str(flg)].append(item)

        # 当任一类型的数据达到20条时，发送该类型的所有数据
        if len(self.thread_local.data_buffer[str(flg)]) >= 20:
            data_to_send = self.thread_local.data_buffer[str(flg)]
            logger.info(f"Sending {len(data_to_send)} items for type {flg}")
            try:
                mongoToMQ(flg, data_to_send)
                data_to_send.clear()
                logger.success(f"【*】发送成功：{len(data_to_send)} items")
            except Exception as e:
                logger.error(f"发送数据失败: {e}")
            finally:
                # 清空已发送的数据
                self.thread_local.data_buffer[str(flg)] = []
        ...
        while True:
            ...
            if res is None:
                ...
                with ThreadPoolExecutor(3) as f:
                    ...
                    _ = 1
                    while True:
                        res=self.local_conn.lpop(self.zi_area_key)
                        if res is None:
                            time.sleep(0.5)
                        else:
                            self.local_conn.lpush(self.zi_area_key+"_01", res)
                            self.local_conn.lpush(self.zi_area_key+"_02", res)
                            self.local_conn.lpush(self.zi_area_key+"_03", res)
                            self.local_conn.lpush(self.zi_area_key+"_04", res)
                            info = json.loads(res.decode("utf-8"))
                            if "_id" in info:
                                del info['_id']
                            print(info)
                            if not self.local_conn.sismember(self.sbxx_filter_key,info["company"]):
                                all_list1.append(f.submit(self.sbxx,info=info,page=1))
                            else:
                            ...
                            if not self.local_conn.sismember(self.zpzzq_filter_key, info["company"]):
                                all_list2.append(f.submit(self.zpzzq, info=info,page=1))
                            else:
                            ...
                            if not self.local_conn.sismember(self.rzzzq_filter_key, info["company"]):
                                all_list3.append(f.submit(self.rzzzq, info=info,page=1))
                            else:
                            ...
                            if not self.local_conn.sismember(self.zzzs_filter_key, info["company"]):
                                all_list4.append(f.submit(self.zzzs, info=info,page=1))
                            else:
                            ...
                            if not self.local_conn.sismember(self.xzxk_filter_key, info["company"]):
                                all_list5.append(f.submit(self.xzxk, info=info,page=1))
                            else:
                            ...
                            if not self.local_conn.sismember(self.dxxk_filter_key, info["company"]):
                                all_list6.append(f.submit(self.dxxk, info=info,page=1))
                            else:
                                logger.warning(f"存在 电信许可 信息:{info}")
                            logger.info(f"。。。。。。。。。。。。。。第 {_} 家公司！！")
                            _+=1
                            for all_list in [all_list1,all_list2,all_list3,all_list4,all_list5,all_list6]:
                                if len(all_list) >= 30:
