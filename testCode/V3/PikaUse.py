...
collection_7 = client["fail_MQ"]["fail_MQ_shareholderInfo"]
...
def mongoToMQ(flg,item_info):
    ...
    channe8 = connection.channel()

    # 创建一个队列，如果不存在的话
    channe1.queue_declare(queue='qqbx.dc.company',durable=True)
    ...
    channe8.queue_declare(queue='qqbx.dc.shareholderInfo',durable=True)

    if flg == 0:
        try:
            # 公司详情
            channe1.basic_publish(exchange='',
        ...
        try:
            # 6 股东
            logger.info(item_info)
            channe8.basic_publish(exchange='',
                    routing_key='qqbx.dc.shareholderInfo',
                    body=json.dumps(item_info))
            logger.success(f"shareholderInfo 股东 数据 {item_info} 发送成功！！")
        except Exception as e:
