...
from detailparam import getdata
...
def filtermap(area):
    if area in ["jiangsu","guangxi","hunan","hubei","jiangxi","jilin"]:
        ...
        return "gsAddr"
    elif area in ["chongqing","yunnan","xinjiang","zhejiang"]:
        return "cqAddr"
    elif area in ["shanxi", "neimenggu", "liaoning","shandong"]:
        return "sdAddr"
...
def proxy_list():
    if config["proxy"]:
        ...
        # 隧道域名:端口号
        tunnel = config["proxy"][2]
        # 用户名密码方式
        username = config["proxy"][0]
        password = "%s:%d" % (config["proxy"][1],l)
        proxies = {
...
class SuccessCODE():
    ...
    def next_page(self,info,page):
        while True:
            try:
                ...
                response = self.session.post(url,params=params,data=str_data,proxies=self.Request["proxy"],timeout=4)
                logger.info(info)
                if "_id" in info:
                    del info["_id"]
                    ...
                    if response.json()["message"] =="":
                        if "data" in response.json() and "items" in response.json()["data"]:
                            ...
                            if page > num:
                                ...
                                self.local_conn.sadd(self.filter_params, info_str)
                                break
                            ...
                            if item_list:
                                try:
                                    ...
                                    logger.success(f"【*】工商保存成功:%s！！"%info["company"])
                                    item_list.clear()
                                except Exception as e:
                                    logger.error(e)
                            if info_list:
                                    try:
                                        ...
                                        logger.success(f"【*1】参数id保存成功:%s！！"%info["company"])
                                    except Exception as e:
                                        logger.error(e)
                                    # try:
    ...
    def senddata(self,item_info,info):
        logger.info(item_info)
        if "_id" in item_info:
            del item_info["_id"]
        self.data1_item.append(item_info["nameLevels"])
        for dd in item_info['labelLists']:
            self.kechuang_item.append(dd)
        ...
        if len(self.kechuang_item) >= 5:
            SendMQ().mongoToMQ(2, self.kechuang_item)
            self.kechuang_item.clear()
        if len(self.data1_item) >= 5:
            SendMQ().mongoToMQ(1, self.data1_item)
            self.data1_item.clear()
        if len(self.data_item) >=5:
            SendMQ().mongoToMQ(0, self.data_item)
            self.data_item.clear()


    def main(self,ua,proxy,mobil):
            ...
            while True:
                try:
                    ...
                    if "company" not in info:
                        info_str = info["areaCode"]+info["base"]+str(info["new_day"])+str(info["next_day"])
                        if not self.local_conn.sismember(self.filter_params, info_str):
...
def main():
    local_VQ_conn = redis.Redis("192.168.5." + config["uAddr"][0],
                                config["uAddr"][1],
                                config["uAddr"][2],
                                config["uAddr"][3],
                                socket_connect_timeout=55)
    with ThreadPoolExecutor(3) as f:
        ...
        for area in area_list:
            ua = get()
            proxy = proxy_list()
            mobil = local_VQ_conn.lpop("searchMobil")
            local_VQ_conn.rpush("searchMobil", mobil)
            mobil = json.loads(mobil)
            sc = SuccessCODE(area)
            futures.append(f.submit(sc.main,ua,proxy,mobil))
