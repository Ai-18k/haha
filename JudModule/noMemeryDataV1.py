...
class NoMemerySpider:
    ...
    def __init__(self):
        ...
        self.coll = self.client_01['sichuan']['商标信息']
        self.coll1 = self.client_01['sichuan']['作品著作权']
        self.coll2 = self.client_01['sichuan']['电信许可']
        self.coll3 = self.client_01['sichuan']['软著著作权']
        self.coll4 = self.client_01['sichuan']['行政许可']
        self.coll5 = self.client_01['sichuan']['资质证书']

        self.cli=self.client['sichuan']['商标信息']
        self.cli1=self.client['sichuan']['作品著作权']
        self.cli2=self.client['sichuan']['电信许可']
        self.cli3=self.client['sichuan']['软著著作权']
        self.cli4=self.client['sichuan']['行政许可']
        self.cli5=self.client['sichuan']['资质证书']

        self.c_fail = self.client['sichuan']['fail_商标信息']
        self.c_fail1 = self.client['sichuan']['fail_作品著作权']
        self.c_fail2 = self.client['sichuan']['fail_电信许可']
        self.c_fail3 = self.client['sichuan']['fail_软著著作权']
        self.c_fail4 = self.client['sichuan']['fail_行政许可']
        self.c_fail5 = self.client['sichuan']['fail_资质证书']
    ...
    def sbxx(self,info,page):
        while True:
            ...
            page+=1
        self.local_0T_conn.sadd("sichuan:filter:sbxx", info["company"])
    ...
    def zpzzq(self,info,page):
        while True:
            ...
            page+=1
        self.local_0T_conn.sadd("sichuan:filter:zpzzq", info["company"])
    ...
    def rzzzq(self,info,page):
        while True:
            ...
            page+=1
        self.local_0T_conn.sadd("sichuan:filter:rzzzq", info["company"])
    ...
    def zzzs(self,info,page):
        while True:
            ...
            page+=1
        self.local_0T_conn.sadd("sichuan:filter:zzzs", info["company"])
    ...
    def xzxk(self,info,page):
        ...
        self.local_0T_conn.sadd("sichuan:filter:xzxk", info["company"])
    ...
    def dxxk(self,info,page):
        while True:
            ...
            page+=1
        self.local_0T_conn.sadd("sichuan:filter:dxxk", info["company"])
        ...
        while True:
            ...
            if res is None:
                ...
                with ThreadPoolExecutor(3) as f:
                    ...
                    _ = 1
                    while True:
                        res = self.local_0T_conn.lpop("sichuan:company_id_04")
                        if res is None:
                            time.sleep(0.5)
                        else:
                            ...
                            if "_id" in info:
                                del info['_id']
                            print(info)
                            if not self.local_0T_conn.sismember("sichuan:filter:sbxx",info["company"]):
                            ...
                            # page=1
                            # self.zpzzq(info)
                            if not self.local_0T_conn.sismember("sichuan:filter:zpzzq", info["company"]):
                            ...
                            # page=1
                            # self.rzzzq(info)
                            if not self.local_0T_conn.sismember("sichuan:filter:rzzzq", info["company"]):
                            ...
                            # page = 1
                            # self.zzzs(info)
                            if not self.local_0T_conn.sismember("sichuan:filter:zzzs", info["company"]):
                            ...
                            # page=1
                            # self.xzxk(info)
                            if not self.local_0T_conn.sismember("sichuan:filter:xzxk", info["company"]):
                            ...
                            # page = 1
                            # self.dxxk(info)
                            if not self.local_0T_conn.sismember("sichuan:filter:dxxk", info["company"]):
