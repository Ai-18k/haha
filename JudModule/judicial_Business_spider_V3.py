...
class SuccessCODE():
    ...
    def __init__(self,area):
        ...
        self.coll_2_1 = self.client[self.config["rkey"]]["法院公告"]
        self.coll_3_1 = self.client[self.config["rkey"]]["经营异常"]
        self.coll_4_1 = self.client[self.config["rkey"]]["限制消费"]
        self.coll_5_1 = self.client[self.config["rkey"]]["裁判文书"]
        self.coll_6_1 = self.client[self.config["rkey"]]["被执行人"]
        self.coll_7_1 = self.client[self.config["rkey"]]["失信被执行人"]
        self.coll_8_1 = self.client[self.config["rkey"]]["行政处罚"]
        self.coll_9_1 = self.client[self.config["rkey"]]["股权冻结"]

        self.col1 = self.client[self.config["rkey"]]["fails_法院公告"]
        self.col1.create_index([('company', ASCENDING)], unique=True)

        self.col2 = self.client[self.config["rkey"]]["fails_经营异常"]
        self.col2.create_index([('company', ASCENDING)], unique=True)

        self.col3 = self.client[self.config["rkey"]]["fails_限制消费"]
        self.col3.create_index([('company', ASCENDING)], unique=True)

        self.col4 = self.client[self.config["rkey"]]["fails_裁判文书"]
        self.col4.create_index([('company', ASCENDING)], unique=True)

        self.col5 = self.client[self.config["rkey"]]["fails_被执行人"]
        self.col5.create_index([('company',ASCENDING)], unique=True)

        self.col6 = self.client[self.config["rkey"]]["fails_失信被执行人"]
        self.col6.create_index([('company', ASCENDING)], unique=True)

        self.col7 = self.client[self.config["rkey"]]["fails_行政处罚"]
        self.col7.create_index([('company', ASCENDING)], unique=True)

        self.col8 = self.client[self.config["rkey"]]["fails_股权冻结"]
        self.col8.create_index([('company',ASCENDING)], unique=True)

        self.headers=dict()
        self.sfaj_item=list()
        self.Request = dict()
        self.area_key=self.config["rkey"]+":company_id_02"
        self.filter_key=self.config["rkey"]+":filter:cpws_com"
                ...
                if response.status_code == 200:
                    if response.json()["state"] == "ok" and response.json()['message'] == "":
                        ...
                        for item in info_list:
                            ...
                            if "_id" in json_data:
                                del json_data["_id"]
                            logger.success(f"【*裁判文书】数据保存成功:{json_data}!!")
    ...
    def toryExecutors(self, info,page):
        while True:
            try:
                ...
                if response.status_code == 200:
                    if "data" in response.json() and response.json()["data"] and response.json()['message'] != "登录后可查看更多" and response.json()["errorCode"] == 0 and response.json()["errorCode"]!="303000":
                            num = math.ceil(int(response.json()["data"]['count']) / 10) if response.json()["data"]['count'] else 0
                            logger.info(f'这个链接共{num}页数据')
                            logger.info(f'当前是第{page}页')
                            if page > num:
                                break
                            ...
                            for item in info_list:
                                ...
                                if "_id" in json_data:
                                    del json_data["_id"]
                                logger.success(f"【*被执行人】数据保存成功:{json_data}!!")
                                logger.info(f"第 {page} 页数据！！")
                                self.send_data(3, json_data)
                                # break
                            page += 1
                        ...
                        if response.json()["data"]:
                            if response.json()['message'] != "登录后可查看更多":
                                ...
                                for item in info_list:
                                    ...
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                                    logger.success(f"【*失信被执行人】数据保存成功:{json_data}!!")
                                    # break
                            else:
                                while True:
                                    res = self.local_VQ_conn.lpop("sifaCookie")
                ...
                if response.status_code == 200:
                    if response.json()['state'] == "ok" and response.json()['message'] == "":
                        ...
                        for item in info_list:
                                ...
                                if "_id" in json_data:
                                    del json_data["_id"]
                                logger.success(f"【*行政处罚】数据保存成功:{json_data}!!")
                                logger.info(f"第 {page} 页数据！！")
                                self.send_data(3, json_data)
                                # break
                    else:
                        ...
                        if response.json()["data"]:
                            if response.json()['message'] == "":
                                ...
                                for item in info_list:
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*股权冻结】数据保存成功:{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                                    # break
                            else:
                        ...
                        if response.json()["data"]:
                            if response.json()['message'] != "登录后可查看更多":
                                ...
                                for item in info_list:
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*经营异常】数据保存成功:{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                                    # break
                            else:
                    ...
                    if response.json()["errorCode"] == 0 and response.json()['message'] != "登录后可查看更多":
                        if response.json()["data"]:
                            ...
                            for item in info_list:
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*限制消费】数据保存成功:{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                                    # break
                        else:
