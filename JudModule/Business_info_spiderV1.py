...
class SuccessCODE():
    ...
    def __init__(self):
        ...
        self.coll_2 = self.client_01["sichuan"]["历史法院公告"]
        self.coll_3 = self.client_01["sichuan"]["历史经营异常"]
        self.coll_4 = self.client_01["sichuan"]["历史限制消费"]
        self.coll_5 = self.client_01["sichuan"]["历史裁判文书"]
        self.coll_6 = self.client_01["sichuan"]["历史被执行人"]
        self.coll_7 = self.client_01["sichuan"]["历史失信被执行人"]
        self.coll_8 = self.client_01["sichuan"]["历史行政处罚"]
        self.coll_9 = self.client_01["sichuan"]["历史股权冻结"]

        self.coll_2_1 = self.client["sichuan"]["历史法院公告"]
        self.coll_3_1 = self.client["sichuan"]["历史经营异常"]
        self.coll_4_1 = self.client["sichuan"]["历史限制消费"]
        self.coll_5_1 = self.client["sichuan"]["历史裁判文书"]
        self.coll_6_1 = self.client["sichuan"]["历史被执行人"]
        self.coll_7_1 = self.client["sichuan"]["历史失信被执行人"]
        self.coll_8_1 = self.client["sichuan"]["历史行政处罚"]
        self.coll_9_1 = self.client["sichuan"]["历史股权冻结"]
        self.col = self.client["sichuan"]["fail_历史sifa_companyid"]
        self.headers=dict()
        self.sfaj_item=list()
        ...
        while True:
            try:
                ...
                if "_id" in info:
                    del info["_id"]
                self.col.insert_one(info)
        self.local_0T_conn.sadd("sichuan:filter:h_cpws_com", info["company"])
        ...
        while True:
            ...
            if res is None:
                ...
                with ThreadPoolExecutor(3) as f:
                    ...
                    _ = 1
                    while True:
                        res = self.local_0T_conn.lpop("sichuan:company_id_01")
                        if res is None:
                            time.sleep(0.5)
                        else:
                            ...
                            if "_id" in info:
                                del info['_id']
                            print(info)
                            if not self.local_0T_conn.sismember("sichuan:filter:h_cpws_com", info["company"]):
