...
class SuccessCODE():
    ...
    def __init__(self,area):
        ...
        self.Request={}
        self.coll_2 = self.client_01[self.config["rkey"]]["历史法院公告"]
        self.coll_3 = self.client_01[self.config["rkey"]]["历史经营异常"]
        self.coll_4 = self.client_01[self.config["rkey"]]["历史限制消费"]
        self.coll_5 = self.client_01[self.config["rkey"]]["历史裁判文书"]
        self.coll_6 = self.client_01[self.config["rkey"]]["历史被执行人"]
        self.coll_7 = self.client_01[self.config["rkey"]]["历史失信被执行人"]
        self.coll_8 = self.client_01[self.config["rkey"]]["历史行政处罚"]
        self.coll_9 = self.client_01[self.config["rkey"]]["历史股权冻结"]

        self.coll_2_1 = self.client[self.config["rkey"]]["历史法院公告"]
        self.coll_3_1 = self.client[self.config["rkey"]]["历史经营异常"]
        self.coll_4_1 = self.client[self.config["rkey"]]["历史限制消费"]
        self.coll_5_1 = self.client[self.config["rkey"]]["历史裁判文书"]
        self.coll_6_1 = self.client[self.config["rkey"]]["历史被执行人"]
        self.coll_7_1 = self.client[self.config["rkey"]]["历史失信被执行人"]
        self.coll_8_1 = self.client[self.config["rkey"]]["历史行政处罚"]
        self.coll_9_1 = self.client[self.config["rkey"]]["历史股权冻结"]

        self.col = self.client[self.config["rkey"]]["fails_历史法院公告"]
        # self.col.create_index([('company', ASCENDING)], unique=True)

        self.col1 = self.client[self.config["rkey"]]["fails_历史经营异常"]
        # self.col1.create_index([('company', ASCENDING)], unique=True)

        self.col2 = self.client[self.config["rkey"]]["fails_历史限制消费"]
        # self.col2.create_index([('company', ASCENDING)], unique=True)

        self.col3 = self.client[self.config["rkey"]]["fails_历史裁判文书"]
        # self.col3.create_index([('company', ASCENDING)], unique=True)

        self.col4 = self.client[self.config["rkey"]]["fails_历史被执行人"]
        # self.col4.create_index([('company', ASCENDING)], unique=True)

        self.col5 = self.client[self.config["rkey"]]["fails_历史失信被执行人"]
        # self.col5.create_index([('company', ASCENDING)], unique=True)

        self.col6 = self.client[self.config["rkey"]]["fails_历史行政处罚"]
        # self.col6.create_index([('company', ASCENDING)], unique=True)

        self.col7 = self.client[self.config["rkey"]]["fails_历史股权冻结"]
        # self.col7.create_index([('company', ASCENDING)], unique=True)
        self.headers=dict()
        self.sfaj_item=list()
        self.area_key=self.config["rkey"]+":company_id_04"
        self.filter_key=self.config["rkey"]+":filter:h_cpws_com"
                ...
                if response.status_code == 200:
                    if response.json()["errorCode"] == 0 and response.json()['message'] != "登录后可查看更多":
                        ...
                        for item in info_list:
                                ...
                                if "_id" in json_data:
                                    del json_data["_id"]
                                logger.success(f"【*历史裁判文书】数据保存成功:{json_data}!!")
                ...
                if response.status_code == 200:
                    if response.json()["state"] == "ok" and response.json()['message'] == "":
                            ...
                            for item in info_list:
                                ...
                                if "_id" in json_data:
                                    del json_data["_id"]
                                logger.success(f"【*历史法院公告】数据保存成功:{json_data}!!")
                                logger.info(f"第 {page} 页数据！！")
                                self.send_data(3, json_data)
                                # break
                    else:
                    ...
                    if response.json()["state"] =="ok" and response.json()['message'] == "":
                        if response.json()["data"]:
                                ...
                                for item in info_list:
                                    ...
                                    try:
                                        if item["caseCreateTime"]:
                                            record_date = ts.datetime.fromtimestamp(item["caseCreateTime"] / 1000) \
                                                .strftime('%Y-%m-%d') if not isinstance(item["caseCreateTime"],str) else item["caseCreateTime"]
                                        else:
                                            record_date =None
                                    except:
                                        record_date =None
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*历史被执行人】数据保存成功:{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                                    # break
                        else:
                                ...
                                for item in info_list:
                                    ...
                                    try:
                                        record_date = ts.datetime.fromtimestamp(
                                            item["publishdate"] / 1000).strftime('%Y-%m-%d') if item["publishdate"] else None
                                    except:
                                        record_date =None
                                    # 立案日期
                                    try:
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*历史失信被执行人】数据保存成功:{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                                    # break
                        else:
                ...
                if response.status_code == 200:
                    if response.json()["state"] == "ok" and response.json()['message'] == "":
                            ...
                            for item in info_list:
                                ...
                                # 发布日期
                                try:
                                    if item["admin_punish_date"]:
                                        record_date = datetime.strptime(item["admin_punish_date"],
                                                                        "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d") \
                                            if not isinstance(item["admin_punish_date"],str) else item["admin_punish_date"]
                                    else:
                                        record_date = None
                                except:
                                    record_date = None
                                ...
                                if "_id" in json_data:
                                    del json_data["_id"]
                                logger.success(f"【*历史行政处罚】数据保存成功:{json_data}!!")
                                logger.info(f"第 {page} 页数据！！")
                                self.send_data(3, json_data)
                                # break
                    else:
                    ...
                    if response.json()["state"] == "ok" and response.json()['message'] == "":
                        if response.json()["data"]:
                                ...
                                for item in info_list:
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*历史股权冻结】数据保存成功:{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                        else:
                            break
                    ...
                    if response.json()["errorCode"] == 0 and response.json()['message'] != "登录后可查看更多":
                        if response.json()["data"]:
                            ...
                            for item in info_list:
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*历史经营异常】数据保存成功::{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                        else:
                            break
        ...
        while True:
            try:
                    ...
                    if response.json()["errorCode"] == 0 and response.json()['message'] != "登录后可查看更多":
                        if response.json()["data"]:
                                ...
                                for item in info_list:
                                    ...
                                    if "_id" in json_data:
                                        del json_data["_id"]
                                    logger.success(f"【*历史限制消费】数据保存成功:{json_data}!!")
                                    logger.info(f"第 {page} 页数据！！")
                                    self.send_data(3, json_data)
                                    # break
                        else:
                ...
                if "_id" in info:
                    del info["_id"]
                try:
                    self.col2.insert_one(info)
                    print("数据插入成功！")
                except errors.DuplicateKeyError:
                    print("数据已存在，插入失败！")
        self.local_conn.sadd(self.filter_key, info["company"])
                    ...
                    while True:
                        ...
                        if res is None:
                            ...
                            if "_id" in info:
                                del info['_id']
                            print(info)
                            if not self.local_conn.sismember(self.filter_key, info["company"]):
