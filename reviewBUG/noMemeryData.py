...
class NoMemerySpider:
    ...
    def __init__(self):
        ...
        self.coll = self.client_01['zhejiang']['商标信息']
        self.coll1 = self.client_01['zhejiang']['作品著作权']
        self.coll2 = self.client_01['zhejiang']['电信许可']
        self.coll3 = self.client_01['zhejiang']['软著著作权']
        self.coll4 = self.client_01['zhejiang']['行政许可']
        self.coll5 = self.client_01['zhejiang']['资质证书']

        self.cli=self.client['zhejiang']['商标信息']
        self.cli1=self.client['zhejiang']['作品著作权']
        self.cli2=self.client['zhejiang']['电信许可']
        self.cli3=self.client['zhejiang']['软著著作权']
        self.cli4=self.client['zhejiang']['行政许可']
        self.cli5=self.client['zhejiang']['资质证书']

        self.c_fail = self.client['zhejiang']['fail_商标信息']
        self.c_fail1 = self.client['zhejiang']['fail_作品著作权']
        self.c_fail2 = self.client['zhejiang']['fail_电信许可']
        self.c_fail3 = self.client['zhejiang']['fail_软著著作权']
        self.c_fail4 = self.client['zhejiang']['fail_行政许可']
        self.c_fail5 = self.client['zhejiang']['fail_资质证书']
    ...
    def sbxx(self,info,page):
            while True:
                ...
                page+=1
            self.local_conn.sadd("zhejiang:filter:sbxx", info["company"])
    ...
    def zpzzq(self,info,page):
        while True:
            ...
            page+=1
        self.local_conn.sadd("zhejiang:filter:zpzzq", info["company"])
    ...
    def rzzzq(self,info,page):
        while True:
            ...
            page+=1
        self.local_conn.sadd("zhejiang:filter:rzzzq", info["company"])
    ...
    def zzzs(self,info,page):
        while True:
            ...
            page+=1
        self.local_conn.sadd("zhejiang:filter:zzzs", info["company"])
    ...
    def xzxk(self,info,page):
        ...
        self.local_conn.sadd("zhejiang:filter:xzxk", info["company"])
    ...
    def dxxk(self,info,page):
        while True:
            ...
            page+=1
        self.local_conn.sadd("zhejiang:filter:dxxk", info["company"])
    ...
    def main(self):
        while True:
            ...
            if res is None:
                ...
                # currentPage = int(self.local_conn.get("zhejiang:zizhi_num"))
                # currentPage =1.txt
                # pageSize = 1000
                # with ThreadPoolExecutor(3) as f:
                #     all_list = []
                #     for i in range(currentPage,6000):
                #         data = self.cli_5.find({}, {"_id": False}).skip((currentPage - 1.txt) * pageSize).limit(pageSize)
                #         _ = 1.txt
                #         for info in data[_:]:
                #商标
                # info ={"company": "山东蓝海股份有限公司", "id": 32970803}
                # self.sbxx(info,1.txt)
                # if not self.local_conn.sismember("zhejiang:filter:sbxx",info["company"]):
                #     all_list.append(f.submit(self.sbxx,info=info))
                # else:
                #     logger.warning(f"存在 商标 信息:{info}")
                # # 作品著作权
                # info ={"company": "青岛啤酒股份有限公司", "id": 4651392}
                # self.zpzzq(info,1.txt)
                # if not self.local_conn.sismember("zhejiang:filter:zpzzq", info["company"]):
                #     all_list.append(f.submit(self.zpzzq, info=info))
                # else:
                #     logger.warning(f"存在 作品著作权 信息:{info}")
                # # 软件
                # info = {"company": "浪潮集团有限公司", "id": 6923813}
                # self.rzzzq(info,1.txt)
                # if not self.local_conn.sismember("zhejiang:filter:rzzzq", info["company"]):
                #     all_list.append(f.submit(self.rzzzq, info=info))
                # else:
                #     logger.warning(f"存在 软件著作权 信息:{info}")
                # #资质
                # info = {"company": "中建八局第一建设有限公司", "id":271787999}
                # self.zzzs(info,1.txt)
                # if not self.local_conn.sismember("zhejiang:filter:zzzs", info["company"]):
                #     all_list.append(f.submit(self.zzzs, info=info))
                # else:
                #     logger.warning(f"存在 资质证书 信息:{info}")
                # # 行政许可
                # info = {"company": "山东蓝海股份有限公司", "id": 32970803}
                # self.xzxk(info,1.txt)
                # if not self.local_conn.sismember("zhejiang:filter:xzxk", info["company"]):
                #     all_list.append(f.submit(self.xzxk, info=info))
                # else:
                #     logger.warning(f"存在 行政许可 信息:{info}")
                # #电信许可
                info = {"company": "阿里云计算有限公司", "id": 138473506}
                self.dxxk(info,1)
                # if not self.local_conn.sismember("zhejiang:filter:dxxk", info["company"]):
                #     all_list.append(f.submit(self.dxxk, info=info))
                # else:
                #     logger.warning(f"存在 电信许可 信息:{info}")
                # logger.info(f"。。。。。。。。。。。。。。第 {_ + (currentPage-1.txt)*pageSize} 家公司！！")
                #             _ += 1.txt
                #             if len(all_list) >= 200:
                ...
                #             break
                #         currentPage += 1.txt
                #         self.local_conn.set("zhejiang:zizhi_num", currentPage)
