...
class SendMQ:
    def __init__(self):
        # 创建一个队列，如果不存在的话
        self.credentials = pika.PlainCredentials('user','user123')  # 用户名和密码
        self.parameters = pika.ConnectionParameters(host='139.9.70.234',
                    # RabbitMQ服务器地址
                    port=5672,
        ...
        self.channe2 = self.connection.channel()
        self.channe3 = self.connection.channel()
        self.channe4 = self.connection.channel()
        self.channe5 = self.connection.channel()
        self.channe6 = self.connection.channel()
        self.channe7 = self.connection.channel()

        self.channel.queue_declare(queue='qqbx.dc.company', durable=True)
        self.channe2.queue_declare(queue='qqbx.dc.industry', durable=True)
        self.channe3.queue_declare(queue='qqbx.dc.qualification', durable=True)
        self.channe4.queue_declare(queue='qqbx.dc.judicial', durable=True)
