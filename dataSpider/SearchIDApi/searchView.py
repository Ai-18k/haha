...
class SuccessCODE():
    ...
    def get_3(self):
        try:
            ...
            elif res["message"]=="账号存在风险，暂不能操作" or "输入的手机号码与密码不匹配，推荐使用短信登录" or response.status_code==406:
                self.local_VQ_conn.sadd("ErrorMobil", json.dumps(self.Request["mobil"]))
                self.local_VQ_conn.lrem("补idMobil", 1, json.dumps(self.Request["mobil"]))
                mobil = self.local_VQ_conn.lpop("补idMobil")
                self.local_VQ_conn.rpush("补idMobil", mobil)
        ...
        try:
            ...
            if response.status_code==200:
                if '<script id="__NEXT_DATA__" type="application/json">' in response.text:
                    ...
                    print(json_date)
                    # if "companyList" not in json_date:
                    #     logger.error(f"{company}:无对应公司！！！")
                    #     return
                    if json_date["props"]["pageProps"]["dehydratedState"]["queries"][0]["state"]["data"]["state"] != "warn":
                        if json_date["page"] == "/search/[[...option]]":
                            ...
                            datas = json_date["props"]["pageProps"]["dehydratedState"]["queries"][2]["state"]["data"]["data"]["companyList"]
                        else:
    ...
    def main(self):
        with ThreadPoolExecutor(4) as f:
            ...
            if self.is_VIP():
                ...
                # all_data = []
                while True:
                    # res= self.conn.lrange("补id:补companyID",0,-1.txt)
