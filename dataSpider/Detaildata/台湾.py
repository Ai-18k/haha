...
def checkconfig(area):
    ...
    serv_conn = redis.Redis('139.9.70.234', 6379,2, "anbo123", socket_connect_timeout=170)
                ...
                if response.status_code == 200:
                    if response.json()["message"] =="":
                        if "data" in response.json() and "items" in response.json()["data"]:
                            ...
                            if page > num:
                                info_str = info["areaCode"] + info["base"] + str(info["new_day"]) + str(
                                    info["next_day"])
...
def main():
    local_VQ_conn = redis.Redis("192.168.5." + config["uAddr"][0],config["uAddr"][1],config["uAddr"][2],config["uAddr"][3],socket_connect_timeout=1155)
    with ThreadPoolExecutor(2) as f:
        ua = get()
        proxy=proxy_list()
        ...
        area_list=[
            "tianjin","heilongjiang","henan","hainan","sichuan","yunnan",
