...
class Login_module:
            ...
            if "输入的手机号码与密码不匹配，推荐使用短信登录" not in response.text:
                ...
                if res["message"]=="":
                    ...
                    user_info = quote(json.dumps({
                            ...
                            "userId": id,
                            "isExpired": "0"}))
                    self.session.cookies.set("sensorsdata2015jssdkcross",
                            sensorsdata)
                    self.session.cookies.set("tyc-user-info",
                            user_info)
                    self.session.cookies.set("tyc-user-info-save-time",
                            str(int(time.time() * 1000)))
                    self.session.cookies.set("tyc-user-phone",
                            "%255B%252218587162714%2522%255D")
                    self.session.cookies.set("auth_token",token)
                    self.session.cookies.set("ssuid","311563409")
    ...
    def main(self,proxy,ua):
        ...
        self.Reqest["ua"]=ua
        if self.get_3():
            session=self.get_cookie_csrf()
            return session,self.Reqest
        else:
            return False
